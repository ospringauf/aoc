...
//	List<String> data = Util.splitLines(example);
    ...
    Character commonInGroup(List<String> group) {
        return group.map(elf -> HashSet.ofAll(elf.toCharArray())).reduce((e1, e2) -> e1.intersect(e2)).single();
    }
    ...
    void part2() {
        var result = data.grouped(3).map(g -> commonInGroup(g)).map(c -> prio(c)).sum();
