...
public record Pose(Direction heading, Point pos) {
    ...
    /**
     * rotate position and direction, but stay within AxA square
     */
    public Pose rotateTo(Direction target, int A) {
        var d = heading;
        var p = pos;
        var c = Point.of(0, A - 1);

        while (d != target) {
            // rotate left and shift "down"
            p = p.rotLeft().plus(c);
            d = d.left();
        }
        return new Pose(d, p);
    ...
    public Pose modulo(int w, int h) {     
        return new Pose(heading, pos.modulo(w, h));
    }
}
